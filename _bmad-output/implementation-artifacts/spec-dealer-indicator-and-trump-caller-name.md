...
- Match the existing `TrumpIndicator` styling/anchoring conventions (rounded-full pill, team colors via `team-red`/`team-blue` Tailwind tokens, `aria-live="polite"`, `data-testid` attributes).
...
|----------|--------------|---------------------------|----------------|
| Trump picked by red-team player | `trumpCallerSeat=0`, `players[0].username="Marko"` | TrumpIndicator shows suit, red team chip, AND "Marko" | N/A |
...
**Acceptance Criteria:**
- Given a game in `playing` phase with trump picked by seat 0 named "Marko" (red team), when GamePage renders, then both the team chip AND the text "Marko" appear inside `[data-testid="trump-indicator"]`.
