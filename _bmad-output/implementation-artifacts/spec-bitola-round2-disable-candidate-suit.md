...
**Problem:** In Bitola round-2 bidding the active bidder currently sees four enabled suit buttons (S/H/D/C) and may legally pick the same suit that was the round-1 face-up candidate. House rules say that suit is already "spent" — round 2 is meant for choosing a *different* trump than the one that nobody wanted in round 1. Today the UI offers it and the server happily accepts it.
...
- Don't add a tooltip, sub-label, or new i18n string explaining *why* the suit is disabled — the visible candidate card above the grid already communicates it.
