...
| First trick won by red | `handPoints=[12,0]`, `declarationPoints=[0,0]` | Red shows `+12 this hand` under match score; blue secondary line hidden. |
| Trick 1 resolves, declarations award blue 50 | `handPoints=[12,14]`, `declarationPoints=[0,50]` | Red shows `+12 this hand`; blue shows `+64 this hand`. |
...
**Execution:**
- [x] `client/src/features/game/components/ScorePanel.tsx` -- extend prop type with `redHandPotential: number` and `blueHandPotential: number`; render a small secondary line under each team's match score showing `+N {t('game.score.thisHand')}` when `>0`, hidden otherwise
- [x] `client/src/features/game/GamePage.tsx` -- pass `redHandPotential={gameState.handPoints[0] + gameState.declarationPoints[0]}` and `blueHandPotential={gameState.handPoints[1] + gameState.declarationPoints[1]}`
...
**Acceptance Criteria:**
- Given it is mid-hand and red has won one trick worth 12 points, when the game state updates, then the ScorePanel shows red match score unchanged and a smaller `+12 this hand` underneath the red row.
- Given belot is announced by a blue-team player, when the game state updates, then blue's "this hand" number increases by 20 without any change to match score.
...
```
┌──────────────────┐
│ Red        120   │  <- unchanged: team-red color, 3xl bold, tabular-nums
...
**Manual checks:**
- Start a game, watch a trick resolve. Red or blue should gain a visible `+N this hand` line immediately without waiting for hand-end.
...
- Secondary render block for red; muted text, tabular-nums, right-aligned for number stability.
  [`ScorePanel.tsx:67`](../../client/src/features/game/components/ScorePanel.tsx#L67)

- Same for blue — independent condition, so one team can show while the other hides.
...
- Four ScorePanel tests covering hide-at-zero, red-only, both-teams independent, default-props.
