  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {/* Close */}
        <DialogClose
          aria-label={t("lobby.roomLobby.ownerConfirm.close", { defaultValue: "Close" })}
          style={{
            position: "absolute",
            top: 16,
            right: 16,
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ marginTop: 2, fontSize: 12, color: "var(--ink-mute)" }}>
          <span style={{ fontFamily: "var(--font-mono)", letterSpacing: 0.4 }}>
            {seated
              ? t("lobby.roomLobby.ownerConfirm.seatLabel", { n: (seat as number) + 1 })
              : t("lobby.roomLobby.ownerConfirm.standing")}
          </span>
        </div>
      </div>
      {seated && relation && (
        <Badge tone={relation === "partner" ? "teamA" : "teamB"}>
          {t(
            relation === "partner"
              ? "lobby.roomLobby.seatTile.partner"
              : "lobby.roomLobby.seatTile.opponent",
          )}
        </Badge>
      )}
    </div>
...
export function KickPlayerDialog({
  ...
  return (
    <OwnerConfirmDialog
      ...
      tone="danger"
      eyebrow={t("lobby.roomLobby.ownerConfirm.eyebrow")}
      icon={<UserX size={24} color="var(--danger)" />}
      titleTestId="kick-dialog-title"
      title={
        <Trans
          i18nKey="lobby.roomLobby.kickConfirm.title"
      ...
      lede={
        <Trans
          i18nKey="lobby.roomLobby.kickConfirm.lede"
          values={{ name: target.name }}
          components={{ strong: <strong style={{ color: "var(--ink)" }} /> }}
        />
      }
      confirmLabel={t("lobby.roomLobby.kickConfirm.confirm")}
      confirmIcon={<UserX size={15} color="#fbeae6" />}
      onConfirm={onConfirm}
      confirmPending={pending}
      confirmTestId="kick-confirm"
      cancelLabel={t("lobby.roomLobby.kickConfirm.cancel")}
...
export function TransferOwnershipDialog({
  ...
  const capabilities = [
    {
      ...
      label: t("lobby.roomLobby.ownerConfirm.capabilities.start"),
    },
    {
      icon: <Shuffle size={13} color="var(--brass-deep)" />,
      label: t("lobby.roomLobby.ownerConfirm.capabilities.seats"),
    },
    {
      icon: <UserX size={13} color="var(--brass-deep)" />,
      label: t("lobby.roomLobby.ownerConfirm.capabilities.players"),
    },
    {
      icon: <Settings size={13} color="var(--brass-deep)" />,
      label: t("lobby.roomLobby.ownerConfirm.capabilities.settings"),
    },
  ];
  ...
  return (
    <OwnerConfirmDialog
      ...
      title={t("lobby.roomLobby.transferConfirm.title")}
      lede={
        <Trans
          i18nKey="lobby.roomLobby.transferConfirm.lede"
          values={{ toName: target.name }}
          components={{ strong: <strong style={{ color: "var(--ink)" }} /> }}
        />
      }
      confirmLabel={t("lobby.roomLobby.transferConfirm.confirm")}
      confirmIcon={<Crown size={15} color="var(--brass-ink)" />}
      onConfirm={onConfirm}
      confirmPending={pending}
      confirmTestId="transfer-confirm"
      cancelLabel={t("lobby.roomLobby.transferConfirm.cancel")}
    ...
    >
      ...
      >
        ...
        >
          ...
          <span style={{ ...captionStyle, color: "var(--ink-mute)" }}>
            {t("lobby.roomLobby.ownerConfirm.youHost")}
          </span>
        </div>
        ...
        >
          ...
          <span style={{ ...captionStyle, color: "var(--brass-deep)", fontWeight: 700 }}>
            {t("lobby.roomLobby.ownerConfirm.newHost", { name: target.name })}
          </span>
        </div>
      </div>
      ...
      >
        {t("lobby.roomLobby.ownerConfirm.handOver")}
      </span>
